...
from pris3.deep3d_facerecon.util import util
from pris3.deep3d_facerecon.util.nvdiffrast import MeshRenderer
from pris3.deep3d_facerecon.util.preprocess import estimate_norm_torch
...
class FaceReconModel(BaseModel):
    ...
    def forward(self):
        ...
        self.pred_vertex, self.pred_tex, self.pred_color, self.pred_lm, self.pred_lm_468= \
    ...
    def save_depth(self, name):
        depth = self.pred_depth.permute(0,2,3,1)
        depth = np.squeeze(depth.cpu().numpy()[0])
        #cv2.imwrite(name, np.array(depth, np.uint8))
        ind = np.where(depth > 0)
        depth_max = np.max(depth[ind])
        depth_min = np.min(depth[ind])
        depth[ind] = 255 - (depth[ind] - depth_min)/(depth_max-depth_min+1e-8) * 255
        cv2.imwrite(name, np.array(depth, np.uint8))
        return  depth
    ...
    def get_coeff(self):
        ...
        pred_lm = np.stack([pred_lm[:,:,0],self.input_img.shape[2]-1-pred_lm[:,:,1]],axis=2) # transfer to image coordinate
        pred_coeffs['lm68'] = pred_lm
        pred_lm_468 = self.pred_lm_468.cpu().numpy()
        pred_coeffs['lm468'] = np.stack([pred_lm_468[:,:,0], self.input_img.shape[2]-1-pred_lm_468[:,:,1]], axis=2)
        return pred_coeffs

    def get_depth_lm468_edit_exp(self, coeff):
        self.facemodel.to(self.device)
        pred_vertex, pred_tex, pred_color, pred_lm, pred_lm468 = \
            self.facemodel.compute_for_render(torch.tensor(coeff).to(self.device))
        pred_mask, pred_depth, pred_face = self.renderer(
            pred_vertex, self.facemodel.face_buf, feat=pred_color) 
        depth = np.squeeze(pred_depth.cpu().numpy()[0])
        ind = np.where(depth > 0)
        depth_max = np.max(depth[ind])
        depth_min = np.min(depth[ind])
        depth[ind] = 255 - (depth[ind] - depth_min)/(depth_max-depth_min+1e-8) * 255
        depth = np.array(depth, np.uint8)
        depth = Image.fromarray(depth).convert("L")
        pred_lm468 = pred_lm468.cpu().numpy()
        pred_lm468 = np.stack([pred_lm468[:, :, 0], self.input_img.shape[2] - 1 - pred_lm468[:, :, 1]], axis=2)
        return depth, pred_lm468

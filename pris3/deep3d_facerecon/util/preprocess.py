...
def resize_n_crop_img(img, lm, t, s, target_size=224., mask=None):
    ...
    coords_crop = [w0, h0, s, left, up, right, below]

    return img, lm, mask, coords_crop
...
def align_img(img, lm, lm3D, mask=None, target_size=224., rescale_factor=102.):
    ...
    img_new, lm_new, mask_new, coords_crop = resize_n_crop_img(img, lm, t, s, target_size=target_size, mask=mask)
    
    trans_params = np.array([w0, h0, s, t[0].item(), t[1].item()])
    crop_params = np.array(coords_crop)

    return trans_params, img_new, lm_new, mask_new, crop_params
